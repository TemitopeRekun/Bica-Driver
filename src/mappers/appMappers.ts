...
export const mapUser = (backendUser: any): UserProfile => ({
  ...
  subAccountActive: backendUser.subAccountActive ?? !!backendUser.monnifySubAccountCode,
  canRetrySubAccountSetup: backendUser.canRetrySubAccountSetup ?? (!backendUser.subAccountActive && !!backendUser.bankName && !!backendUser.accountNumber),
  approvalStatus: backendUser.approvalStatus || (backendUser.status === 'PENDING' || backendUser.status === 'APPROVED' || backendUser.status === 'REJECTED' ? backendUser.status : undefined) || 'PENDING',
